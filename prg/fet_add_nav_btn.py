...
class ClasseNavBtn:
    ...
    def update_js_and_css(self):
        ...
        if len(self.in_path_file_name) != 0:

            # create the out_file_name with the in_file_name
            if "_js" not in self.in_path_file_name:
                self.out_file_name = os.path.basename(self.in_path_file_name).replace(".js", "_wnav.epub")
            else:
                self.out_file_name = os.path.basename(self.in_path_file_name)
            ...
            u.manage_info("".join(["Add nav btn : ", datetime.now().strftime("%Y%m%d-%H%M%S")]), u.DISPLAY_AND_LOG)
    ...
    def file_update_js_and_css(self):
        ...
        if len(str(self.in_path_file_name)) > 0:
            ...
            self.update_js_and_css_job()  # start the optimisation
        else:
    ...
    def change_police(self):
        ...
        if len(self.in_path_file_name) != 0:
            ...
            # create the out_file_name with the in_file_name
            if "_pol" not in self.in_path_file_name:
                self.out_file_name = os.path.basename(self.in_path_file_name).replace(".epub", "_pol.epub")
            else:
                self.out_file_name = os.path.basename(self.in_path_file_name)

            # the out filename goes in the wnav directory
            self.out_path_file_name = "".join([self.self.new_pol_path, self.out_file_name])
            # prepare the text to display in and out files
            self.txt_in_file = "".join(["Sce file : ", os.path.basename(self.in_path_file_name)])
            self.txt_out_file = "".join(["Dst file : ", self.out_path_file_name])

            # display the files status
            u.manage_info("".join(["Change police : ", datetime.now().strftime("%Y%m%d-%H%M%S")]), u.DISPLAY_AND_LOG)
            u.manage_info(self.txt_in_file, u.DISPLAY_AND_LOG, u.COLOR_BLUE)
            u.manage_info(self.txt_out_file, u.DISPLAY_AND_LOG, u.COLOR_BLUE)
            u.manage_info("", u.DISPLAY_AND_LOG)
    ...
    def file_change_police(self):
        ...
        if len(str(self.in_path_file_name)) > 0:
            self.t_start = time.time()  # store the start time
            self.in_file_name = os.path.basename(self.in_path_file_name)
            self.change_police()  # start the optimisation
        else:
            self.msg_list.insert(tk.END, "Select a epub to improve")
            self.msg_display.update()
        u.manage_buttons("btnAddNav", "out")
    ...
    def dir_change_police(self):
        ...
        # demande le nom du répertoire à travailler
        file_options = {}
        file_options['initialdir'] = self.wnav_path #self.org_path
        file_options['title'] = 'Please select a directory witch content epub files'
        dir_name = filedialog.askdirectory(**file_options)
    ...
    def file_prepare_for_moodle(self):
        ...
        if len(str(self.in_path_file_name)) > 0:
            self.t_start = time.time()  # store the start time
            self.in_file_name = os.path.basename(self.in_path_file_name)
            self.prepare_for_moodle()  # start the optimisation
        else:
            self.msg_list.insert(tk.END, "Select a epub to improve")
            self.msg_display.update()
        u.manage_buttons("btnAddNav", "out")
    ...
    def prepare_for_moodle(self):
        ...
        if len(self.in_path_file_name) != 0:
            ...
            # create the out_file_name with the in_file_name
            if "_moo" not in self.in_path_file_name:
                self.out_file_name = os.path.basename(self.in_path_file_name).replace(".epub", "_moo.epub")
            else:
                self.out_file_name = os.path.basename(self.in_path_file_name)
                    ...
                    for l in xhtml_data:
                        ...
                        if "</script>" in l and in_body:
                            ...
                            for js_in_file in os.listdir(self.js_path_dir):
                                ...
                                if "alternateur" in js_in_file or "cercle" in js_in_file or "oscillo" in js_in_file or "validation" in js_in_file:
    ...
    def file_1_2_3_4(self):
        ...
        if len(str(self.in_path_file_name)) > 0:
            self.t_start = time.time()  # store the start time
            self.in_file_name = os.path.basename(self.in_path_file_name)
            # self.add_nav_btn_job()  # start the optimisation
            # self.update_js_and_css_job()  # start the optimisation
            # self.change_police()  # start the optimisation
            # self.prepare_for_moodle()  # start the optimisation
        else:
    ...
    def remove_nav_btn(self, in_path_file_name):
        ...
        if len(in_path_file_name) != 0:
            ...
            # create the out_file_name with the in_file_name
            if "_wonav" not in in_path_file_name:
                self.out_file_name = os.path.basename(in_path_file_name).replace(".epub", "_wonav.epub")
            else:
                self.out_file_name = os.path.basename(in_path_file_name)
