...
function showAlertDialog(correct, qId) {
    ...
    // begin added section by jo ======================================================
    // new exo so attemps restart to 0
    if (last_exo_id != "answer"+qId) {
        attemps = 0;
    }
    //  end added section by jo ======================================================
    //dialog
    alertDialog = document.createElement('div');
    alertDialog.id = 'alertDialog' + qId;
    alertDialog.style.cssText = "position:fixed; width:80%; top: 5%; left: 10%; background: #6b6a63; padding: 3px;";

    //	alertDialog.style.cssText = "position:fixed; width:80%; top: 5%; left: 5%; margin-left:5%; margin-top: 5%; background: #6b6a63; padding: 3px;";
    //content
    contentDiv = document.createElement('div');
    contentDiv.style.cssText = "background:#fff;padding:20px;text-align:center;";
    contentDiv.appendChild(document.createTextNode(text+"\n(essai no "+String(attemps+1)+")"));
    //buttons
...
function resetExercise(qId) {
    ...
    if (functionName.indexOf("checkbox") >= 0) {
		...
		for(i=0; i<params.length; i++)  {   
			if (String(params[i]).search("answer") != -1){
				param = params[i];
				break;
			}
		}
		resetValueInRange(param);
    } else {
...
function toggleHelp(qId) {
    ...
    if (hint.style.display.indexOf("none") == 0)
        hint.style.cssText = "widht:20%; position:center; border-width:thin;";
...
function showHint(qId) {
    ...
    alertDialog = document.createElement('div');
    alertDialog.id = 'alertDialog' + qId;
    alertDialog.style.cssText = "position:fixed; width:40%; top: 5%; left: 30%; background: #6b6a63; padding: 3px;";
...
function resetValueInRange(id) {
	if (typeof(id) != undefined && id != null) {	
		...
		var input = document.getElementById(last_exo_id);
	}
	input.value = '0';
}
