...
function onResizeAction () {
	...
	w0 = parseInt(0.95*window.innerWidth);
	h0 = parseInt(0.95*window.innerHeight);
	rw = w0 / w1 ;
	rh = h0 / h1 ;
	r_scale_factor = 1.03; 
	...
	// debugPrint0("w0="+w0, "w1="+w1, "rw="+rw.toFixed(2), "h0="+h0, "h1="+h1, "rh="+rh.toFixed(2), "rf="+rf.toFixed(2),"canvas_w="+ctx.canvas.width, "canvas_h="+ctx.canvas.height);
...
function refreshGraph(){
	if (monophase_YN){
		...
		txt_ph2.style.display = "block";
		txt_ph3.style.display = "block";
...
function drawStator(ctx, r, s, p, c, w) {
    // dessiner le stator
	// debugPrint0("r="+parseInt(r), "p="+parseInt(p)); // ,"Rx="+parseInt(Rx), "s="+parseInt(s), "r="+parseInt(r), "Xmin="+parseInt(Xmin), "Xmax="+parseInt(Xmax), "Xlen="+parseInt(Xlen));
...
function drawRotor(ctx, R, s, p) {
	// debugPrint1("Rr="+parseInt(Rr), "R="+parseInt(R),"Rx="+parseInt(Rx), "s="+parseInt(s), "r="+parseInt(r), "Xmin="+parseInt(Xmin), "Xmax="+parseInt(Xmax), "Xlen="+parseInt(Xlen));
    // dessiner le rotor
    ...
    var letter_style = letter_height + "px Verdana"
    ...
    ctx.stroke();
	// debugPrint2("Rr="+parseInt(Rr), "R="+parseInt(R),"Rx="+parseInt(Rx), "s="+parseInt(s), "r="+parseInt(r), "Xmin="+parseInt(Xmin), "Xmax="+parseInt(Xmax), "Xlen="+parseInt(Xlen));
}
...
function draw_sinus(ctx, Rr, R, ph1) {
    ...
    ph = 360;
    document.getElementById("txt_deg_id").innerHTML = "phi = " + ph2 + "˚";
    ...
    ctx.beginPath();
    ctx.lineWidth = l_width / 2;
    var letter_height = (l_width * 12).toFixed();
    ...
    for (i = 1; i != 9; i++) {
		...
		if (valeurs_YN) {
			ctx.strokeText(45 * i, Xgrad, 4 * gradLen);
		}
    }
    Xgrad = 8.3 * (Xmin + Xlen) / 8;
	ctx.strokeText("φ", Xgrad, 4 * gradLen);
    ...
    for (i = -4; i != 5; i++) {
		...
		if (valeurs_YN) {
			if (tension_YN) {
				ctx.strokeText(-0.25 * i * u_crete, -5 * gradLen, Ygrad + gradLen / 2);
			} else {
				ctx.strokeText(-0.25 * i, -5 * gradLen, Ygrad + gradLen / 2);
			}
		}
    }
	Ygrad = -4 * Ymax / 4;
	if (tension_YN) {
		ctx.strokeText("U", +2 * gradLen, (Ygrad - 2*gradLen ));
	}
    ...
    if (!monophase_YN) {
        ...
        for (i = 0; i < ph; i++) {
            ...
            Xnew = i * (Xmin + Xlen) / 360;
            var Yrad = i * Math.PI / 180;
            var Ysin = -Math.sin(Yrad + 2 / 3 * Math.PI);
            Ynew = Ysin * Ymax;
            ctx.moveTo(Xold, Yold);
            ctx.lineTo(Xnew, Ynew);
            Xold = Xnew;
        ...
        for (i = 0; i < ph; i++) {
            ...
            Xnew = i * (Xmin + Xlen) / 360;
            var Yrad = i * Math.PI / 180;
            var Ysin = -Math.sin(Yrad + 4 / 3 * Math.PI);
            Ynew = Ysin * Ymax;
            ctx.moveTo(Xold, Yold);
            ctx.lineTo(Xnew, Ynew);
            Xold = Xnew;
    ...
    if (tension_YN) {
        document.getElementById("txt_ph1_id").innerHTML = "U=" + -(326 * Ysin).toPrecision(3)+ "V";
    } else {
        document.getElementById("txt_ph1_id").innerHTML = "ph1=" + -(Ysin).toPrecision(3);
    }
    ctx.fill();
    // draw_sin cos lines
    if (lines_YN) {
    ...
    if (!monophase_YN) {
        ...
        if (tension_YN) {
            document.getElementById("txt_ph3_id").innerHTML = "U=" + -(326 * Ysin).toPrecision(3)+ "V";
        } else {
            document.getElementById("txt_ph3_id").innerHTML = "ph3=" + -(Ysin).toPrecision(3);
        }
        ctx.fill();
        // draw_sin cos lines
        if (lines_YN) {
        ...
        if (tension_YN) {
             document.getElementById("txt_ph2_id").innerHTML = "U=" + -(326 * Ysin).toPrecision(3)+ "V";
        } else {
           document.getElementById("txt_ph2_id").innerHTML = "ph2=" + -(Ysin).toPrecision(3);
        }
        ctx.fill();
        // draw_sin cos lines
        if (lines_YN) {
...
// function debugPrint0(txt1="", txt2="", txt3="", txt4="", txt5="", txt6="", txt7="", txt8="", txt9="", txt10=""){
	// document.getElementById("d0").innerHTML = txt1 + "  " + txt2 + "  " + txt3 + "  "  + txt4 + "  "  + txt5 + "  "  + txt6 + "  "  + txt7 + "  "  + txt8 + "  "  + txt9 + "  "  + txt10 ;
// }
// function debugPrint1(txt1="", txt2="", txt3="", txt4="", txt5="", txt6="", txt7="", txt8="", txt9="", txt10=""){
	// document.getElementById("d1").innerHTML = txt1 + "  " + txt2 + "  " + txt3 + "  "  + txt4 + "  "  + txt5 + "  "  + txt6 + "  "  + txt7 + "  "  + txt8 + "  "  + txt9 + "  "  + txt10 ;
// }
// function debugPrint2(txt1="", txt2="", txt3="", txt4="", txt5="", txt6="", txt7="", txt8="", txt9="", txt10=""){
	// document.getElementById("d2").innerHTML = txt1 + "  " + txt2 + "  " + txt3 + "  "  + txt4 + "  "  + txt5 + "  "  + txt6 + "  "  + txt7 + "  "  + txt8 + "  "  + txt9 + "  "  + txt10 ;
// }
