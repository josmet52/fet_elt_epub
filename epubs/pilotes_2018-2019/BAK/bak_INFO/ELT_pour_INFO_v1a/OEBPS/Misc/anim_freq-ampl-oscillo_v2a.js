...
function displayValue() {
    ...
    periode = (1000 / freq).toPrecision(4);
    ...
    if (freq < 10) {
        lbl_freqval.innerHTML = "Fréquence = " + freq + " Hz<br/>Période = " + periode/1000 + " s";
        // lbl_freq.innerHTML = "Fréquence<br/>[" + String(ms_div*1000) + " s/div]";
        ...
        lbl_freqval.innerHTML = "Fréquence = " + freq/1000 + " kHz<br/>Période = " + periode*1000 + " us";
        // lbl_freq.innerHTML = "Fréquence<br/>[" + String(ms_div*1000) + " us/div]";
        lbl_freq.innerHTML = "Fréquence";
    }
...
function btnDivPlusNew() {
    if (ms_div / periode <= 5) {
        l_list = ms_div_list.length;
        if (ms_div < ms_div_list[0]) {
            for (i = 0; i < l_list; i++) {
                if (ms_div == ms_div_list[i]) {
                    ms_div = ms_div_list[i - 1];
                    break;
                }
            }
        }
        if (ms_div > 50) {
            lbl_div.innerHTML = (ms_div / 1000) + " s/div";
        } else if (ms_div > 0.1) {
            lbl_div.innerHTML = ms_div + " ms/div";
        } else {
            lbl_div.innerHTML = (ms_div * 1000) + " us/div";
        }

    }
}
function btnDivMoinsNew(){
    l_list = ms_div_list.length;
    if (ms_div > ms_div_list[l_list-1]){
        for (i=0; i < l_list; i++) {
            if (ms_div == ms_div_list[i]) {
                ms_div = ms_div_list[i+1];
                break;
