...
function onResizeAction() {
    ...
    if (show_logo) {
        cel_logo.style.display = "table-cell";
    } else {
        cel_logo.style.display = "none";
    }

    var f_reduction = 0.8;
    var w0 = (f_reduction * window.innerWidth);
    var h0 = w0 * 0.5 ; // (f_reduction * window.innerHeight);
    r_base_x = w0;
    r_base_y = h0 - cel_tension.offsetHeight;

    canvas_element.height = r_base_y;
    canvas_element.width = r_base_x;
    ...
    r_display_x = (r_base_x - 2 * xy_margin - 2 * line_width);
    x_0 = (r_base_x / 2);
    x_min = (-r_display_x / 2);
    x_max = (r_display_x / 2);

    r_display_y = (r_base_y - 2 * xy_margin - 2 * line_width);
    y_0 = (r_base_y / 2);
    y_min = (-r_display_y / 2);
    y_max = (r_display_y / 2);
