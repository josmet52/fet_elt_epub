...
function showAlertDialog(correct, qId){
	var text = correct ? "Congratulations! Answer is correct." : "The anwer is wrong";
	//dialog
	...
	buttons.appendChild(createButton("Reset", "resetExercise", qId, true));
	...
	if (correct){
		buttons.appendChild(createButton("Close", "", qId, true));
		attemps = 0;
	}else{
		if (attemps < 2){
			buttons.appendChild(createButton("Try again", "", qId, true));
		}
		attemps++;
	}
